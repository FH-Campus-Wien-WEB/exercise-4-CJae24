...
app.put("/movies/:imdbID", requireLogin, function (req, res) {
  ...
  if (!exists) {
    // Task 2.3: Fetch movie data from OMDb and save it
    ...
    fetch(url, { signal: controller.signal })
      .then(apiRes => {
        ...
        return apiRes.text().then(data => {
          ...
          // Convert OMDb format to internal format
          const internalMovie = {
            imdbID: omdbMovie.imdbID,
            Title: omdbMovie.Title,
            Year: isNaN(omdbMovie.Year) ? null : parseInt(omdbMovie.Year),
            Released: omdbMovie.Released || '',
            Runtime: isNaN(omdbMovie.Runtime) ? null : parseInt(omdbMovie.Runtime),
            Directors: omdbMovie.Director ? omdbMovie.Director.split(', ') : [],
            Writers: omdbMovie.Writer ? omdbMovie.Writer.split(', ') : [],
            ...
            Metascore: isNaN(omdbMovie.Metascore) ? null : parseInt(omdbMovie.Metascore),
            imdbRating: isNaN(omdbMovie.imdbRating) ? null : parseFloat(omdbMovie.imdbRating),
          };
